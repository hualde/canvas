...
export async function exportEmpathyMapToPDF(canvas: EmpathyMapData) {
  if (!canvas) {
    ...
    const addContent = (items: string[], x: number, y: number, maxWidth: number, maxHeight: number) => {
      if (Array.isArray(items)) {
        ...
        items.forEach((item) => {
          ...
          if (cleanedItem) {
            const lines = doc.splitTextToSize(cleanedItem, maxWidth - 5); // Reduce maxWidth slightly to account for bullet point
            doc.text('•', x, currentY);
